...
def Display_hash(file: str, md5: bool, sha1: bool, sha256: bool):
    """display hash"""
...
def main():
    ...
    parser.add_argument(
        "-1",
        "--sha1",
        help="Display the sha1 file content",
        action="store_true",
    )
    ...
    parser.add_argument(
        "-256",
        "--sha256",
        help="Display the sha2566 file content",
        action="store_true",
    )
    ...
    if md5 is False and sha1 is False and sha256 is False:
        print("{!} you must use these (md5, sha1, sha256) switches")
        sys.exit()
    ...
    if os.path.isfile(path=path):
       Display_hash(file=path, md5=md5, sha1=sha1, sha256=sha256)
    else:
        if recursive:
        ...
        for file in files:
            Display_hash(file=file, md5=md5, sha1=sha1, sha256=sha256)    
